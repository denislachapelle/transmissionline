...
\begin{equation}
	A0\, U(t) + A1\, \frac{\partial U(t)}{\partial t} - A2\, h(t) = 0
\end{equation}
...
\begin{equation}
	U(t_n) = U(t_{n-1}) + \Delta t A1^{-1} (A2\, h(t_n) - A0 U(t_n))
\end{equation}

\begin{equation}
   A1\,	U(t_n) = A1\, U(t_{n-1}) + \Delta t\, A2\, h(t_n) - \Delta t\, A0\, U(t_n)
\end{equation}

Term with $t_n$ on LHS and term with $t_{n+1}$ on RHS.

\begin{equation}
	(A1+ \Delta t\, A0)	U(t_n) = A1\, U(t_{n-1}) + \Delta t\, A2\, h(t_n)
\end{equation}

shift n by +1

\begin{equation}
	(A1+ \Delta t\, A0)	U(t_{n+1}) = A1\, U(t_n) + \Delta t\, A2\, h(t_n)
\end{equation}
...
\newpage
The next multiplot shows a sine wave propagating through the transmission line, 100m rg-58 cable. The source is 50ohm the load is 2 ohm, we can see the reflection and the current inverting and the attenuation.\\
\includegraphics[height=12cm,keepaspectratio]{./2x5-sine.png}\\
