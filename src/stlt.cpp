...
{
   cout << deltaT << " deltaT\n";
   cout << 1.0/sqrt(L*C) << " 1.0/sqrt(L*C)\n";
...
/*
   ...
   K3smM_I->ScaleRow(nbrVDof-1, 0.0);
   K4smS_I->ScaleRow(nbrIDof-1, 0.0);
   K4smS_I->Set(nbrIDof-1, nbrIDof-1, Rl);
*/
...
// 6. Define the BlockStructure of lhsMatrix
   ...
   (*lhsRowBlockOffset)[1]=nbrIDof; 
   (*lhsRowBlockOffset)[2]=nbrVDof; 
...
// 6. Define the BlockStructure of lhsMatrix
   ...
   (*rhsRowBlockOffset)[1]=nbrIDof; 
   (*rhsRowBlockOffset)[2]=nbrVDof; 
   ...
   VDofByOne = new DenseMatrix(nbrIDof, 1);
...
// Build the operator, insert each block.
   ...
   rhsOp->SetBlock(0, 1, K3smM_I);
   rhsOp->SetBlock(0, 2, K4smS_I);
   rhsOp->SetBlock(1, 1, K1smS_V);
   rhsOp->SetBlock(1, 2, K2smM_V);
