...
#define AIR        (nbrGroups+1)
#define AIRCONTOUR (nbrGroups+2)
...
class ProximityEffect
{
   ...
   private:
      ...
      FiniteElementCollection *Afec;
      FiniteElementSpace *Afespace;
      ...
      FiniteElementCollection *Bfec;
      FiniteElementSpace *Bfespace;
      GridFunction *BrGF, *BiGF, *BGF;  // magnetic field, real and imaginary.

      FiniteElementCollection *BNormfec;
      FiniteElementSpace *BNormfespace;
      GridFunction *BnGF;  // magnetic field, real and imaginary.
   ...
   public:
      void SetNbrGroups(int);
      void SetGroupsInfo(GroupInfo*);
...
void ProximityEffect::SetNbrGroups(int n) {nbrGroups=n;}
void ProximityEffect::SetGroupsInfo(GroupInfo *gi) {groupsInfo = gi;}
...
{
   Afec = new H1_FECollection(order, dim);
   Afespace = new FiniteElementSpace(mesh, Afec);
   nbrdof = Afespace->GetNDofs(); 
   cout << Afespace->GetNDofs() << " degree of freedom\n"
        << Afespace->GetVDim() << " vectors dimension\n\n";   
